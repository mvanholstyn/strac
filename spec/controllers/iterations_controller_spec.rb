...
# TODO - the implementation of #stories renders rjs, how to test with rspec? 
describe IterationsController, "#stories on an actual iteration requested by user with privileges" do
  def get_stories
    get :stories, { :project_id => 1, :id => 2 }, { :current_user_id => 3 }
  end
  ...
  before do
    ...
    @iteration_presenter = mock "iteration presenter"
    ...
    @project.should_receive(:backlog_iteration).and_return(@backlog)
    @project.should_receive(:iterations).twice().and_return(@iterations)
    IterationsPresenter.should_receive(:new).with(:iterations => @iterations, :backlog => @backlog)
    IterationPresenter.should_receive(:new).with(@iteration).and_return(@iteration_presenter)
    @iterations.should_receive(:find).with('2').and_return(@iteration)
    @iteration_presenter.should_receive(:stories).and_return(@stories)

    get_stories
  end

  it "assigns @iteration" do
    assigns[:iteration].should == @iteration_presenter
  end
  ...
  it "renders the iterations/stories template" do
    response.should render_template("iterations/stories")
  end
end

describe IterationsController, "#stories on a backlog iteration requested by user with privileges" do
  def get_stories
    get :stories, { :project_id => 1, :id => 'backlog' }, { :current_user_id => 3 }
  end
  ...
  before do
    ...
    @project.should_receive(:backlog_iteration).twice().and_return(@backlog)
    @project.should_receive(:iterations).and_return(@iterations)
    IterationsPresenter.should_receive(:new).with(:iterations => @iterations, :backlog => @backlog)
    BacklogIterationPresenter.should_receive(:new).with(@backlog).and_return(@iteration_presenter)
    @iteration_presenter.should_receive(:stories).and_return(@stories)
