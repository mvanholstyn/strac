...
describe "/stories/_edit.html.erb" do
  def render_it
    render :partial => "stories/edit", :locals => { :story => @story }
  end
  ...
  it "renders an edit form" do
    render_it
    response.should have_tag('form#edit_form')
  end
  ...
  def in_the_edit_form(&block)
    response.should have_tag('form#edit_form[action=?]', story_path(@project, @story)), &block
  end
